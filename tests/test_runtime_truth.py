...
def test_receipt_evidence_validation_claim_outcome_telemetry():
    ...
    outcome = record_outcome(db, intent_id=e.intent_id, actor_id="a", runtime_id="r", authority_ref="auth", outcome={"result": "completed"}, evidence_refs=["test://receipt"])
    assert outcome.event == "OUTCOME" and outcome.state_after == "REAL"
    telemetry = record_telemetry(db, intent_id=e.intent_id, actor_id="a", runtime_id="r", telemetry={"latency_ms": 1})
    assert telemetry.event == "TELEMETRY" and telemetry.state_after == "REAL"
...
def test_state_before_cannot_override_ledger():
    ...
    append_event(db, intent_id=e.intent_id, actor_id="a", runtime_id="r", event="ACCEPT", state_after="ACCEPTED", authority_ref="auth")
    try:
        append_event(db, intent_id=e.intent_id, actor_id="a", runtime_id="r", event="START", state_after="STARTED", state_before="INTENDED")
    except ValueError as exc:
        assert "State-before mismatch" in str(exc)
    else:
        raise AssertionError("state-before override accepted")
