...
def _event(e):
    return {"event_id": e.event_id, "intent_id": e.intent_id, "receipt_id": e.receipt_id, "event": e.event,
            "state_before": e.state_before, "state_after": e.state_after, "timestamp": e.timestamp.isoformat(),
            "event_hash": e.event_hash, "previous_hash": e.previous_hash, "evidence_refs": e.evidence_refs,
            "authority_ref": e.authority_ref, "replayable": e.replayable}


@router.get("/health")
def runtime_health(db: Session = Depends(get_db)):
    return {"status": "ok", "ledger": "runtime_events"}


@router.post("/intents")
def intents(body: IntentIn, db: Session = Depends(get_db)):
    return _event(create_intent(db, **body.model_dump()))


@router.post("/intents/{intent_id}/events")
def events(intent_id: str, body: EventIn, db: Session = Depends(get_db)):
    try:
        return _event(append_event(db, intent_id=intent_id, **body.model_dump()))
...
def evidence(intent_id: str, body: EvidenceIn, db: Session = Depends(get_db)):
    try:
        return _event(record_evidence(db, intent_id=intent_id, **body.model_dump()))
    except ValueError as exc:
...
def recovery(intent_id: str, body: RecoverIn, db: Session = Depends(get_db)):
    try:
        return _event(recover(db, intent_id=intent_id, **body.model_dump()))
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
def intent_state(intent_id: str, db: Session = Depends(get_db)):
    try:
        result = replay(db, intent_id)
        result["ledger_valid"] = verify_ledger(db, intent_id)
        return result
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
